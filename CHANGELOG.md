...
- Multi-monitor placement: resolve per-screen work areas through the
  documented two-argument `clientArea` overload (the three-argument
  call returned the union area of all screens, centering the dropdown
  across a boundary); `show()` now guarantees the window fully on the
  active screen while preserving manually placed windows.
